const styles = {
  ...
  dateInput: ({ hasDate }: { hasDate: boolean }) => `border border-gray-200 rounded p-2 text-gray-950 w-full focus:outline-none text-left ${!hasDate ? 'opacity-60' : ''}`,
  ...
  btnWrapper: (showCal: boolean) => `mt-2 mb-6 border-2 rounded border-white ${showCal ? 'border-violet-700' : ''}`,
